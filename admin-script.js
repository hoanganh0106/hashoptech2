...
// Test Cloudflare Images API connection
async function testCloudflare() {
    try {
        showNotification('Đang test kết nối Cloudflare Images...', 'info');
        
        const response = await fetch(`${API_BASE}/admin/test-cloudflare`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${authToken}`,
                'Content-Type': 'application/json'
        ...
        if (data.success) {
            showNotification('✅ Kết nối Cloudflare Images thành công!', 'success');
            ...
            statusDiv.innerHTML = `
                ...
                <div style="font-size: 0.9rem; margin-top: 4px; color: #6b7280;">
                    Account ID: ${data.data.accountId}<br>
                    Token: ${data.data.tokenPreview}<br>
                    Variants: ${data.data.availableVariants.join(', ')}<br>
                    Total Images: ${data.data.totalImages}
                </div>
            `;
            ...
            showNotification(`❌ Lỗi Cloudflare Images: ${data.error}`, 'error');
            
            // Hiển thị thông tin lỗi
            statusDiv.innerHTML = `
                <div style="color: #ef4444; font-weight: 500;">
                    <i class="fas fa-exclamation-circle"></i> Kết nối thất bại
                </div>
                <div style="font-size: 0.9rem; margin-top: 4px; color: #6b7280;">
                    Account ID: ${data.data?.accountId || 'N/A'}<br>
                    Token: ${data.data?.tokenPreview || 'N/A'}<br>
                    Error Code: ${data.data?.errorCode || 'Unknown'}
                </div>
            `;
            statusDiv.style.display = 'block';
        ...
        showNotification('❌ Lỗi kết nối Cloudflare Images', 'error');
